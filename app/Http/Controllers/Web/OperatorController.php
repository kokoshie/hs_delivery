...
class OperatorController extends Controller
{
	...
	protected function find_point_result(Request $request)
	{
		...
		// dd($package->id);
		$kg_price = PackageKg_Price::where('package_id',24)->where('min_kg','<=',$request->weight)->where('max_kg','>=',$request->weight)->first();
		// dd($kg_price);
		if($kg_price == null)
		{
	...
	protected function store_wayplan_now(Request $request)
	{
		...
		$store_wayplan = WayPlanSchedule::create([
			...
			'receive_address'=> $request->receive_addr,
			'receive_date'=> $request->receive_date,
			'dropoff_point'=> $request->drop_point,
			'dropoff_address'=> $request->drop_addr,
